    ...
    export default {
        data(){
            return {
                ...
                alphaMin:0.5 ,// 根据不同材质，透明度不同;假设我们的材质最小alpha值是该值（球背面离我们最远的那个点的alpha值）
                mousePos:[], // 鼠标坐标值
                rotateLimit:1 ,// 转动速度，限制值2度；
        ...
        methods:{
            ...
            calcPonit(){ // 从所有的切面上，围绕切面（圆）的圆周，生成点；
                ...
                // y=rsinθsinφ.
                // z=rcosθ.
                // 只需随机N个 θ 和 φ （范围是0到360）；代入到公式中，生成xyz即可：
                for(let i=0;i<900;i++){
                    // 随机两个0到360之间的角度值：
                    let a = Math.random()*360 ;
                    let b = Math.random()*360 ;
                    // 代入公式：
                    let x = this.ball.R*Math.sin(Math.PI/180*a)*Math.cos(Math.PI/180*b) ;
                    let y = this.ball.R*Math.sin(Math.PI/180*a)*Math.sin(Math.PI/180*b) ;
                    let z = this.ball.R*Math.cos(Math.PI/180*a) ;
        ...
        mounted(){
            this.init() ; // 初始化开始状态 ；
            // this.calcLayer() ;  // 计算并获得切面的基本数据 // 随机点的话，就不需要切面了
